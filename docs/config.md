...
In this guide, we explain how to integrate the WSO2 API Store with an external Identity and Access Management (IAM) by using the Okta OAuth Authorization Server, 
which is an open source IAM, to manage the OAuth clients and tokens required by WSO2 API Manager. This is a sample client implementation that consumes APIs exposed by Okta OAuth.
...
1. Go to [Okta sign up page](https://developer.okta.com/signup/). Create an Okta account and get the Okta instance URL.

   Eg :- [https://dev-76342239.oktapreview.com](https://dev-76342239.oktapreview.com)
...
2. Get the authorization server Id by following the steps below :
    ...
    1. Go to the **API** tab and select the **Authorization Servers** section.
    2. Create a new authorization server or you can use the default server.
        ![alt text](images/authorization_server.png)

3. Get the API key :
    
    1. Log in to the created account and go to the **API** tab and then select **Tokens**.
...
4. Create Access Policies : If you already have atleast one access policy in your authorization server, please skip the following steps and go to `step 1: (5)`.
    ...
    5. Choose Create Policy to save the policy.
    
5. Create Rules for Access Policy : If you already have atleast one rule in your access policy, please skip the following steps and go to `step 1: (6)`.
...
>> **Note :** Make sure that you replace the `<oktaInstanceUrl>` value with the client registration endpoint that you obtained in step 1: (1) and the `<apiKey>` value with the API key obtained in `step 1: (3)`.
...
3. **Create an application :**
    1. Store UI :
       ...
       Go to the API Store and click on **Applications tab**. Then click on **ADD APPLICATION** to create a new application.
...
4. **Generate an Application Key :** 
    ...
    1. Store UI :
       ...
       After create an application, go to Production Keys tab of the Application, then click the Generate Keys button.
...
6. **Update Grant Types :** 
    ...
    Edit the application details in the Okta.
    
    1. Store UI: 
    
        Go to Production Keys tab of the Application, fill out the values to be updated and then click on Update.
...
7. **Delete an OAuth Application :** 
    ...
    Delete an OAuth application in the Okta server.
...
8. **Provision an Out-of-Band OAuth Client :** Provision an OAuth client created in the Okta server.
    ...
    Enable the option to provide out-of-band keys by opening the `<APIM_HOME>/repository/deployment/server/jaggeryapps/store/site/conf/site.json` file and changing the `"mapExistingAuthApps"` setting to `true`.
    ...
    1. Store UI : 
        ...
        Click Provide Keys under Map Existing OAuth Keys.
         ...
         >>**Note :** If you haven't provide consumer secret, the access token will not be generated.
...
9. **Clean Partially Created Keys :** 
    ...
    1. Store UI : 
        ...
        Go to the **Production Keys** tab of the Application then click **Clean up**.
...
10. **Revoke the token and re-generate the access token from the OAuth Provider :**
    ...
    2. CURL command :
        1. Update the token endpoint in the `<APIM_HOME>/repository/deployment/server/synapse-configs/default/api/_TokenAPI_.xml` file accordingly.
        ...
        2. Update the revoke endpoint in the `<APIM_HOME>/repository/deployment/server/synapse-configs/default/api/_RevokeAPI_.xml` file accordingly.
        ...
        3. Replace `<ConsumerKey:ConsumerSecret>` with the `Base64 encoded ConsumerKey:ConsumerSecret` of the client application you just created.
            ...
            After revoking the access token, you need to follow below step to regenerate the new access token.
        
        4. Let's obtain a token from the OAuth Provider.
